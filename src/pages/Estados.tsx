...
const EstadosPage: React.FC = () => {
  const [itens, setItens] = React.useState<Cidade[]>([])
  ...
  const carregar = React.useCallback(async () => {
    ...
    try {
      ...
      // tentar normalizar
      const arr: Cidade[] = Array.isArray(data) ? data : data?.data || []
      setItens(arr)
    } catch (e: any) {
  ...
  return (
    <>
      <Helmet>
        <title>Estados | SoloVivo Gestão</title>
        <meta name="description" content="Gerencie estados e assentos vinculados." />
      ...
      <main className="container py-6">
        ...
        <section className="grid md:grid-cols-2 gap-6">
          <article className="border rounded-lg p-4 bg-card">
            <h2 className="font-medium mb-4">Novo Estado e Assentamento</h2>
